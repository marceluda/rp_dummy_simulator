...
Implementa un filtro pasabajos de primer orden (equivalente a un filtro RC).
...
Definición del tiempo característico del pasabajos (T=RC):
...
module filtro_pasabajos #(parameter R=14, TAU_OFFSET=14)
    (
    input                  clk,rst,  // Clock and reset
    input         [ 4-1:0] tau,      // Tau, tiempo característico del filtro pasabajos
...
filtro_pasabajos #(.R(14),.TAU_OFFSET(14)) i_filtro_pasabajos_NAME (
    .clk(clk), .rst(rst), 
    // inputs
    .tau( 14'd12   ), 
    .dis( 1'b0     ), 
...
filtro_pasabajos #(.R(14),.TAU_OFFSET(14)) i_filtro_pasabajos_NAME (.clk(clk),.rst(rst), .tau( 14'd12   ), .dis( 1'b0 ), .in(  IN  ),.out(  OUT  ) );

filtro_pasabajos i_filtro_pasabajos_NAME (.clk(clk),.rst(rst), .tau( 14'd12   ), .dis( 1'b0 ), .in(  IN  ),.out(  OUT  ) );
